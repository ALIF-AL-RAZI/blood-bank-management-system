...
<br><br><br>
	...
	<input id="a_id" type="text" name="a_id" placeholder="Your ID"  style="width: 150px;height: 30px;border: none; border-bottom: 2px solid aqua;"/>
...
<?php
 ...
 $q=$db->prepare("INSERT INTO request(a_id,h_id,test_date) VALUES (:a_id,:h_id,:test_date)");
 ...
 $q->bindValue('test_date',$test_date);
      ...
      if($q->execute())
            {
              echo "<script>alert('Request Registered Successfull')</script>";
            }
            else
            {
              echo "<script>alert('Request Registered Unsuccessfull')</script>";
            }
         } 
           ?>
